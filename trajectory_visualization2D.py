...
def main():
    ...
    for idx, trajectory_file_path in enumerate(args.trajectory_files):
        ...
        plt.plot(trajectory.positions_xyz[:, 0], trajectory.positions_xyz[:, 1], label=f'Trajectory {idx + 1}', color=color)

        if args.left:
            plt.scatter(left_points[:, 0], left_points[:, 1], label=f'Left Points {idx + 1}', color='red', s=10)
        if args.right:
            plt.scatter(right_points[:, 0], right_points[:, 1], label=f'Right Points {idx + 1}', color='green', s=10)
        if args.plot:
            plt.scatter(left_points[:, 0], left_points[:, 1], label=f'Left Points {idx + 1}', color='red', s=10)
            plt.scatter(right_points[:, 0], right_points[:, 1], label=f'Right Points {idx + 1}', color='green', s=10)
    ...
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.title('Trajectories with Furthest Left and Right Points')
    plt.show()
